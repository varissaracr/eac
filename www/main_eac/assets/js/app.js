...
let checkurid = (id) => {
    // console.log(id)
    if (id !== null) {
        $('#profile').html(`<a href="./../form_authen/profile/index.html"><i class="bx bxs-user-detail"></i><span>ข้อมูลผู้ใช้งานระบบ</span></a>`)
        $('#login').html(`<a href="#" onclick="Logout()"><i class="bx bx-log-out"></i><span>ออกจากระบบ</span></a>`)
    } else {
        $('#login').html(`<a href="./../form_authen/login/index.html"><i
        class="bx bx-exit"></i><span>เข้าสู่ระบบ</span></a>`)

    }
}
let Logout = async () => {
    await sessionStorage.removeItem("ustoken")
...
let getProv_group = async (prov) => {
    $('#MP_group').modal('show')
    $('#M_head').text(`รายชื่อกลุ่ม เครือข่าย องค์กรในจังหวัด${prov}`)
    await $('#myTable').DataTable().destroy();
    ...
    let dtable = $('#myTable').DataTable({
            ...
            { data: 'orgstatus' },
            {
                ...
                render: function (data, type, row, meta) {
                    if (data.geojson !== null) {
                        ...
                        return `<span class="text-danger"><b>ไม่พบตำแหน่งที่ตั้ง</b></span>`
                    }
                },
            },
        ],
        columnDefs: [
            ...
            {
                render: function (data, type, full, meta) {
                    return "<div class='text-wrap text-center'>" + data + "</div>";
                },
                targets: 0
            },
            {
...
let onEachFeature = (feature, layer) => {
    ...
    if (feature.properties) {
        let name = feature.properties.orgname;
...
function customTip() {
    // console.log(this.feature.geometry.properties.orgname)
    let txt = `<strong class="kanit">${this.feature.geometry.properties.orgname}</strong>`
    this.unbindTooltip();
