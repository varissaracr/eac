...
$(document).ready(function () {
    ...
    table = $('#myTable').DataTable({
        ajax: {
            type: "get",
            url: "https://engrids.soc.cmu.ac.th/api" + `/elephant-eac/getalldata`,
            dataSrc: 'data',
        },
        columns: [
            {
            ...
            },
            {
                ...
                render: function (data, type, row, meta) {
                    return `${row.elephant1 !== 'ไม่มี' ? row.elephant1 + "" : ""}
                    ${row.elephant2 !== 'ไม่มี' ? row.elephant2 + "" : ""}
                    ${row.elephant3 !== 'ไม่มี' ? row.elephant3 + "" : ""}
                    ${row.elephant4 !== 'ไม่มี' ? row.elephant4 + "" : ""}
                    ${row.elephant5 !== 'ไม่มี' ? row.elephant5 : ""}
                    ${row.elephant1 == 'ไม่มี' && row.elephant2 == 'ไม่มี' && row.elephant3 == 'ไม่มี' &&
                            row.elephant4 == 'ไม่มี' && row.elephant5 == 'ไม่มี' ? '-' : ""}`
                },
            ...
            { data: 'record' },
            {
                ...
                render: function (data, type, row, meta) {
                    ...
                    return `<button class= "btn m btn-loaction" id = "getzoomMap" ><i class="fas fa-map-marker-alt"></i>&nbsp;ตำแหน่ง</button>
                            ...
                            <button class="btn m btn-danger" onclick="confirmDelete(${row.id_date},'${row.num_elephant}','${row.charact_ele}','${row.record}')"><i class="bi bi-trash"></i>&nbsp;ลบข้อมูล</button>`
                },
                // width: "30%"
            }
...
let d
let getdata = (id) => {
    axios.get(url + `/elephant-eac/getalldata`, { id_user: id }).then(async (r) => {
        d = r.data.data;
        // console.log(d)
...
let getMap = (x) => {
    ...
    x.map(i => {
        if (i.geojson) {
            ...
            let elephant = `${i.elephant1 !== 'ไม่มี' ? i.elephant1 + "" : ""}
        ${i.elephant2 !== 'ไม่มี' ? i.elephant2 + "" : ""}
        ${i.elephant3 !== 'ไม่มี' ? i.elephant3 + "" : ""}
        ${i.elephant4 !== 'ไม่มี' ? i.elephant4 + "" : ""}
        ${i.elephant5 !== 'ไม่มี' ? i.elephant5 : ""} `
            geojson
                .bindPopup(`< h6 class= "text-center" ><b>รายละเอียด</b>: ${elephant} <br><b>จำนวนช้างป่าที่พบ(ตัว)</b> ${i.num_elephant} <br><b>ลักษณะช้างป่าที่พบ</b> ${i.charact_ele} <br><b>ที่ตั้ง</b>: ต.${i.tam_tn} อ.${i.amp_tn} จ.${i.prov_tn} <br>
...
let zoomMap = (x) => {
    var a = [x];
    let elephant = `${a[0].elephant1 !== 'ไม่มี' ? a[0].elephant1 + "" : ""}
                            ${a[0].elephant2 !== 'ไม่มี' ? a[0].elephant2 + "" : ""}
                            ${a[0].elephant3 !== 'ไม่มี' ? a[0].elephant3 + "" : ""}
                            ${a[0].elephant4 !== 'ไม่มี' ? a[0].elephant4 + "" : ""}
                            ${a[0].elephant5 !== 'ไม่มี' ? a[0].elephant5 : ""}`
    var pop
    if (a[0].num_elephant) {
        pop = L.popup({ Width: 200, offset: [0, -25] });
        let content = `<h6 class="text-center"><b>รายละเอียด</b>: ${elephant} <br><b>จำนวนช้างป่าที่พบ (ตัว)</b>: ${a[0].num_elephant} <br><b>ลักษณะช้างป่าที่พบ</b>: ${a[0].charact_ele} <br><b>ที่ตั้ง</b>: ต.${a[0].tam_th} อ.${a[0].amp_th} จ.${a[0].prov_th} <br>
    ...
    d.map(i => {
        if (i.geojson) {
            ...
            mm = L.geoJson(json, {
                ...
                .bindPopup(`<h6><b>จำนวนช้างป่าที่พบ (ตัว)</b>: ${i.num_elephant} </h6> <h6><b>ลักษณะช้างป่าที่พบ</b>: ${i.charact_ele} </h6><br><b>ที่ตั้ง</b>: ต.${i.tam_th} อ.${i.amp_th} จ.${i.prov_th} <br>
            ...
            mm = L.marker([i.lat, i.lng], { name: "marker" , icon: MIcon_05})
                .bindPopup(`<h6><b>จำนวนช้างป่าที่พบ (ตัว)</b>: ${i.num_elephant} </h6> <h6><b>ลักษณะช้างป่าที่พบ</b>: ${i.charact_ele} </h6> <h6><b>จังหวัด :</b> ${i.prov_th}</h6> <h6><b>ผู้ให้ข้อมูล :</b> ${i.record}</h6>
            `)
            ms.addLayer(mm);// 
        }
    });
...
let confirmDelete = (id_data, num_elephant,) => {
    $("#projId").val(id_data)
    $("#num_elephant").html(`${num_elephant !== 'null' ? 'จำนวนช้างป่าที่พบ (ตัว)' + num_elephant : ''} `)
...
let deleteValue = () => {
    ...
    axios.post(url + "/elephant-eac/delete", { proj_id: id_data }).then(r => {
